...
# Функция для автоматического коммита изменений
def commit_and_push_changes():
    try:
        repo = Repo(".")
        repo.git.add(DATA_FILE)
        repo.index.commit("Update user data")
        origin = repo.remote(name="origin")
        origin.push()
        logging.info("Изменения успешно закоммичены и отправлены на GitHub.")
...
async def venom(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Сохранение данных и коммит изменений
    save_user_data()
